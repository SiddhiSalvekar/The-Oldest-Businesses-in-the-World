...
SELECT business, year_founded, country_code, categories.category
FROM businesses 
...
SELECT MIN(bus.year_founded) AS oldest, coun.continent 
FROM businesses AS bus
INNER JOIN countries AS coun
  ON bus.country_code = coun.country_code
GROUP BY continent
ORDER BY oldest
```
...
SELECT bus.business, bus.year_founded, cat.category, coun.country, coun.continent
FROM businesses AS bus
INNER JOIN categories AS cat USING(category_code)
INNER JOIN countries AS coun USING(country_code)
...
SELECT coun.continent, cat.category, COUNT(*) AS n
FROM businesses AS bus
INNER JOIN categories as cat
  ON bus.category_code = cat.category_code
INNER JOIN countries as coun
  ON bus.country_code = coun.country_code
GROUP BY coun.continent, cat.category
```
